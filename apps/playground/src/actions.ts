...
export function createActions(client: MonitorClient, writeLog: (message: string) => void): PlaygroundAction[] {
 return [
  { id: "js-error", label: "JS Error", run: () => triggerJsError() },
  { id: "rejection", label: "unhandledrejection", run: () => triggerUnhandledRejection() },
  { id: "console-error", label: "console.error", run: () => console.error("playground console error") },
  { id: "xhr-ok", label: "XHR 成功", run: () => requestWithXhr("/api/pv?source=xhr-ok") },
  { id: "xhr-fail", label: "XHR 失败", run: () => requestWithXhr("/__missing") },
  { id: "fetch-ok", label: "fetch 成功", run: () => fetch("/api/pv?source=fetch-ok", { method: "POST" }) },
  { id: "fetch-fail", label: "fetch 失败", run: () => fetch("/__missing", { method: "POST" }) },
  { id: "resource-error", label: "资源加载失败", run: () => triggerResourceError() },
  { id: "manual-api", label: "手动 API", run: () => fetch("/rapi/metricjts", createJsonRequest({ manual: true })) },
  { id: "manual-error", label: "手动 error", run: () => client.reportError(new Error("manual playground error")) },
  { id: "metric", label: "metric", run: async () => reportMetric(client) },
  { id: "reset-pv", label: "resetPv", run: () => client.resetPv({ pageId: "playground-reset" }).reportPv() },

...
describe("ErrorManager", () => {
  ...
  it("发送失败期间新入队错误不会被缓存清理误删", async () => {
    ...
    const send = vi.fn<SendFn>(
      () =>
        new Promise<TransportResponse | void>((_resolve, reject) => {
          rejectSend = reject;
        }),
    );
    const storage = createStorage();
  ...
  describe("parsePromiseUnhandled ", () => {
  ...
  describe("monitorErrDetected 事件 ", () => {
    it("有效错误入队时广播 CustomEvent", async () => {
      ...
      try {
        ...
        expect(event.type).toBe("monitorErrDetected");
  ...
  describe("detectLeave ", () => {
    it("注册 beforeunload 后页面离开时尝试 sendBeacon", () => {
      ...
      try {
        ...
        // 模拟 beforeunload
        const handler = (globalThis.window as unknown as { onbeforeunload: (() => void) | null })
          .onbeforeunload;

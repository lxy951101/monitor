...
export class ErrorManager {
  ...
  // 限流相关 (对齐 owl.js time-window rate limiting)
  private errorCount = 0;
  private timeLimit = Date.now();
  private isTimeLimited = false;

  // 在途请求追踪 (对齐 owl.js cacheSending)
  ...
  /**
   * 解析 window.onerror 采集到的 JS 错误，对齐 owl.js parseWindowError。
   */
  parseWindowError(
    msg: string | Event,
    url?: string,
  ...
  /**
   * 处理 unhandledrejection 错误，对齐 owl.js parsePromiseUnhandled。
   */
  ...
  /**
   * 解析 console.error 参数，对齐 owl.js parseConsoleError。
   */
    ...
    try {
      ...
      for (const arg of args) {
        ...
        if (typeof arg === "string") {
          ...
          arg instanceof ErrorEvent
        ) {
          msg =
            (arg.error && (arg.error.stack || arg.error.message)) ||
            arg.message ||
            "";
        } else {
  ...
  /**
   * 快捷上报方法，对齐 owl.js report() = push + send(true)。
   */
  ...
  async sendErrors(isReportNow = true): Promise<void> {
    ...
    // 限流检查 (对齐 owl.js time-window rate limiting)
    ...
    try {
      ...
      // 处理远程配置下发 (对齐 owl.js handleRemoteConfig)
  ...
  /**
   * 延迟读取并上报 localStorage 中历史上报失败后存储的异常 (对齐 owl.js checkCache)。
   */
  checkCache(): void {
    setTimeout(() => {
  ...
  handlePageLeave(): void {
    ...
    // 尝试 sendBeacon (对齐 owl.js detectLeave)
  ...
  /**
   * 自动注册页面离开检测 (对齐 owl.js detectLeave)。
    ...
    try {
      ...
      window.onbeforeunload = (() => {
        ...
        if (!cacheDisabled || useSendBeacon) {
          // 1. 未进入上报流程的队列
          let cacheCombined = this.queue.map((item) =>
            this.addCacheExtension(item.model)
          );
            ...
            for (const [, record] of this.cacheSending.entries()) {
              try {
                ...
                if (record.cache.length) {
                  const models = record.cache.map((item) =>
                    this.addCacheExtension(item.model)
                  );
  ...
  /**
   * SDK 自身错误上报 (对齐 owl.js reportSystemError)。
   */
  ...
  /**
   * SDK 自身 warn 上报 (对齐 owl.js reportSystemWarn)。
   */
  ...
  /**
   * 内部 push 管线 (对齐 owl.js push 方法中的过滤、事件、去重、hook)。
   */
  private pushModel(model: ErrorModel): void {
    // 采样检查 (对齐 owl.js isSampleHit)
    ...
    // ignoreList (string 前缀匹配，对齐 owl.js ignoreList.js)
  ...
  /**
   * 内容去重：判断错误是否已存在当前队列中 (对齐 owl.js isExist)。
   */
  ...
  /**
   * 限流检查 (对齐 owl.js send 中的 maxNum/maxTime 窗口)。
   */
  ...
  /**
   * 合并 extension 数据到错误模型 (对齐 owl.js addCacheExtension)。
   */
      ...
      if (typeof window !== "undefined" && window.dispatchEvent) {
        ...
        if (typeof window.CustomEvent === "function") {
          ...
          if (typeof evt.initCustomEvent === "function") {
            evt.initCustomEvent("owlErrDetected", false, false, eventInfo);
  ...
  private applyFilters(model: ErrorModel): ErrorModel | undefined {
    try {
      ...
      return beforeSendResult === false || beforeSendResult === null
        ? undefined
        : beforeSendResult;
    } catch {
      return model;
  ...
  private enqueue(item: QueueItem): void {
    ...
    // combo 模式：延迟合并发送；非 combo 模式：立即发送 (对齐 owl.js combo 开关)
    if (this.getCombo()) {
      this.scheduleFlush();
    } else {
      this.flushInBackground();
  ...
  /**
   * 构建 sendBeacon URL (对齐 owl.js detectLeave 中的 pageId/beacon 参数)。
   */
  ...
  /**
   * 处理服务端下发的远程配置 (对齐 owl.js handleRemoteConfig)。
   */

...
export interface FirstScreenObserverOptions {
  ...
  /** 停止监听的超时时间（ms），默认 3000。对齐 owl.js mutaStopTime。 */
  stopTime?: number;
  /** 首屏外 mutation 计数阈值，达到后提前停止。默认 15。 */
  maxOutCount?: number;
  /** 忽略的元素属性名，包含该属性的元素不计入首屏。默认 "owl-ignore"。 */
  ignoreAttr?: string;
...
/**
 * 启动首屏观察器（对齐 owl.js FirstScreenManager）。
 *
 * 工作流程：
...
export function startFirstScreenObserver(
  ...
  const ignoreAttr = options.ignoreAttr ?? "owl-ignore";

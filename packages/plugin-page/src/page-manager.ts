...
export interface PageManagerOptions extends CoreConfigPatch {
  ...
  pageUrl?: string;
  realUrl?: string;
  /** 上报延迟（ms），用于防抖合并。默认 0 表示立即上报。与 delay 一致。 */
  delay?: number;
...
export class PageManager {
  ...
  getPerformanceTiming(): {
    perf?: NavigationTimingLike;
    paint?: ReturnType<typeof getPaintEntries>;
  } {
  ...
  parsePageTime(senseTime?: { FST?: number; FCP?: number }): void {
    ...
    if (senseTime) {
      ...
      this.points[25] = senseTime.FST && senseTime.FST > fcp ? senseTime.FST : fcp;
      this.points[26] = fcp;
    }
  ...
  addPoint(point: { position: number; duration?: number }): void {
    if (!point || typeof point.position !== "number" || point.position < 0 || point.position > 31) {
      return;
    }
   ...
   */
  private doSend(pts: number[], customPts: number[], overridePageUrl?: string): void {
    this.clearTimeout();
    ...
    const pageUrl =
      ...
      getPageUrl();
    const project = (pts.length ? this.initialProject : undefined) ?? cfg.project;

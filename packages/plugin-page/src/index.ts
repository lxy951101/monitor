...
export interface PagePluginOptions
 extends Omit<Partial<PageManagerOptions>, "send" | "cfgManager"> {
 env?: {
  performance?: {
 ...
 return {
  ...
  start(context: MonitorContext) {
   ...
   if (shouldReportPage(context)) {
    ...
    const timing =
     options.env?.performance?.timing ?? getRuntimeTiming();
...
function shouldReportPage(context: MonitorContext): boolean {
 ...
 return (
  config.autoCatch.page &&
  config.page.enable &&
  context.cfgManager.isSampled("page")
 );
}
...
function getRuntimeTiming(): NavigationTimingLike | undefined {
 ...
 return (performance as unknown as { timing?: NavigationTimingLike })
  .timing;
}

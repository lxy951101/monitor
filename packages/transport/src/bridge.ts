...
export type BridgeMethod = (params: BridgeRequestParams, callbacks: BridgeCallbacks) => void;
...
export interface ContainerBridgeReporter {
  reportFsp(
    event: Record<string, unknown>,
  ): Promise<{ ok: true; status: 0; body?: unknown; cached?: boolean }>;
}
...
export function createContainerBridgeReporter(
  options: ContainerBridgeReporterOptions,
): ContainerBridgeReporter {
  return {
    async reportFsp(event) {
      const methodName = options.preferredMethod ?? "ffp.record";
      const method = getBridgeMethods(options).find((candidate) =>
        getBridgeMethod(options.bridge, candidate),
      );
      if (!method) {
      ...
      return sendEventWithBridge(
        getBridgeMethod(options.bridge, method) as ContainerBridgeMethod,
        event,
      );
    },
  };
...
function getBridgeMethod(
  bridge: BridgeLike | undefined,
  methodName: string,
): ContainerBridgeMethod | undefined {
...
function saveContainerBridgeCache(
  options: ContainerBridgeReporterOptions,
  item: CachedBridgeEvent,
): void {
...
function writeContainerBridgeCache(
  options: ContainerBridgeReporterOptions,
  items: CachedBridgeEvent[],
): void {
  const storage = getBridgeCacheStorage(options);
  if (!storage) {
    return;
...
function getBridgeCacheStorage(
  options: ContainerBridgeReporterOptions,
...
function isCachedBridgeEvent(value: unknown): value is CachedBridgeEvent {
  ...
  return (
    typeof item.methodName === "string" && Boolean(item.event) && typeof item.event === "object"
  );
}
...
function toBridgeError(error: unknown): Error {
  ...
  return new TransportError(typeof error === "string" ? error : "Bridge request failed");
}

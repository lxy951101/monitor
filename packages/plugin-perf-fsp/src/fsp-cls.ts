...
function clsNodePositionChange(
  state: FspClsRuntimeState,
  elementWithRect: ElementWithRect,
): number {
  ...
  if (!rectsAreEqual(lastRect, rect, state.runtime)) {
    ...
    distance = horizontalMove > verticalMove ? horizontalMove / width : verticalMove / height;
...
function calculateClsScore(state: FspClsRuntimeState, runtime: FspRuntime, distance: number): void {
  ...
  const rect = state.allMovedNodesRects.reduce(
    (acc, item) => ({
...
function rectsAreEqual(left: RectLike, right: RectLike, runtime: FspRuntime): boolean {
  return (
    equalOrPastStart(left.left, right.left) &&

...
export class FspManager {
  ...
  async report(
    ...
    if (this.options.containerBridge) {
      await this.options.containerBridge.reportFsp(
        this.createBridgeEvent(finalMetrics, reportInput),
      );
      return;
    }
    await sendWithPerfCache(
      this.createRequest(finalMetrics),
      this.options.send,
      this.options.cache,
    );
  }
...
export function calculateFsp(input: FspInput): FspMetrics {
  const detectorMetrics = createDetectorMetrics(
    input.detector,
    input.mutationCount,
    input.cls,
    input.costMs,
...
function createFspContainerBridge(
  options: FspPluginOptions,
  preferredMethod: string,
): ContainerBridgeReporter | undefined {
  const bridge =
    options.containerBridge ?? options.runtime?.containerBridge ?? getGlobalContainerBridge();
  if (!bridge) {
    return undefined;
  }

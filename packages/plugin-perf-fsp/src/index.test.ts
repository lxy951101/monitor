...
describe("秒开 2.0", () => {
  it("计算成功、超时和隐藏状态", () => {
    ...
    expect(
      calculateFsp({ startTime: 100, firstScreenTime: 900, now: 900, timeout: 500 }).status,
    ).toBe("timeout");
  ...
  it("容器桥上报 start 和 notsupport 生命周期事件", async () => {
    ...
    (
      runtime.document as { elementsFromPoint?: (x: number, y: number) => Element[] }
...
function createContext(send: ReturnType<typeof vi.fn>, fsp: Record<string, unknown> = {}) {
  return {
    cfgManager: new CfgManager({
      project: "demo",
      perf: {
        fsp: { endpoint: "/perf/fsp", timeout: 1000, defer: false, fspClsEnable: false, ...fsp },
      },
    }),
  ...
  return {
    document: {
      ...
      addEventListener: vi.fn((type, callback) =>
        options.documentAddEventListener?.(type, callback),
      ),
      removeEventListener: vi.fn(),
    },
    innerWidth: 300,
    ...
    clearTimeout: vi.fn(),
    setInterval: vi.fn(
      options.setInterval ?? (() => 2 as unknown as ReturnType<typeof setInterval>),
    ),
    clearInterval: vi.fn(),
  };
}

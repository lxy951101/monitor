...
export function startResourceErrorCapture(
  ...
  const listener = (event: Event) => {
    ...
    // 元素类型过滤 (对齐 owl.js: 仅 script/link/img)
    ...
    // 懒加载图片过滤 (对齐 owl.js: url 等于 href 前缀部分)
    ...
    // resourceReg + ignoreList 过滤 (对齐 owl.js)
...
function filterResourceUrl(
  ...
  try {
    ...
    const ignoreList =
      cfgManager?.getConfig("resource").ignoreList ??
...
/** XPath 提取 (对齐 owl.js Util.getXPath) */

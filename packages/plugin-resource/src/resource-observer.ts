...
import { CSS_PATTERN, IMG_PATTERN, JS_PATTERN, getImageDomain } from "@monitor/protocol";
...
export interface ResourceObserverEnv {
  PerformanceObserver?: new (
    callback: (list: { getEntries: () => ResourceTimingEntryLike[] }) => void,
  ) => {
...
export function startResourceObserver(
  ...
  if (env.PerformanceObserver && !cfgManager?.getConfig("resource").disablePerformanceObserver) {
    const observer = new env.PerformanceObserver((list) => {
      try {
        for (const call of collectResourceEntries(list.getEntries(), cfgManager, options)) {
...
/** 按 initiatorType 过滤 */
function filterEntries(entries: ResourceTimingEntryLike[]): ResourceTimingEntryLike[] {
...
function normalizeType(initiatorType: string, url: string): string {
  ...
  if (initiatorType === "script" || (initiatorType === "link" && JS_PATTERN.test(url))) {
    return "js";
  }
  if (initiatorType === "css" || (initiatorType === "link" && CSS_PATTERN.test(url))) {
    return "css";
  }

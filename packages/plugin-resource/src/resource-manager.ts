...
export class ResourceManager {
  ...
  private doPush(input: ResourceCallInput): void {
    ...
    if (resCfg.combo) {
      ...
      // 非 combo 模式：每条立即发送 (对齐 owl.js report)
      this.triggerReport(true);
    }
  }
  ...
  async sendBatch(): Promise<void> {
    ...
    const buffer = devMode
      ? encodeResourceTextBatch(data)
      : encodeResourceProtobufBatchSafe(data);
    const body: BodyInit =
      typeof buffer === "string" ? buffer : new Blob([buffer as BlobPart]);
    const apiPath = this.cfgManager.getApiPath(
      devMode ? "batchTs" : "pbBatchTs"
    );
  ...
  private handleRemoteConfig(response: TransportResponse | void): void {
    ...
    try {
      ...
      if (body && typeof body === "object" && body.sampling) {
        this.cfgManager.applyRemoteSampling(
          body.sampling as Record<string, number>
        );
      }
    } catch {
      // ignore

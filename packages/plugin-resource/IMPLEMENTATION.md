...
该插件严格对齐 `refer/owl_1.13.5.js` 的 `ResManager` 模块，同时在架构上拆分为 5 个独立模块，并通过依赖注入实现完整可测试性。
...
对齐 owl.js `triggerReport(reportNow)` 模式：
...
入队前回调，可返回 `false` 阻止 push（对齐 owl.js）：
...
- `statusCode` 格式: `"httpCode|businessCode"` (owl.js 对齐)

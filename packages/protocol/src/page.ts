...
export function encodePageSpeed(model: PageSpeedModel): string {
  ...
  points[DERIVED_INDEX.dns] =
    model.dns ?? normalize((model.domainLookupEnd || 0) - (model.domainLookupStart || 0));
  points[DERIVED_INDEX.tcp] =
    model.tcp ?? normalize((model.connectEnd || 0) - (model.connectStart || 0));
  points[DERIVED_INDEX.download] =
    model.download ?? normalize((model.responseEnd || 0) - (model.requestStart || 0));

...
Monitor SDK 的数据协议层，定义各上报通道的数据模型、字段顺序和编码格式，与后端接口及 `refer/owl.js` (v1.13.5) 保持一致。
...
**对应 owl.js**: `OwlError` 类及其 `toJson()` / `encodeDataBeforeSend()` 方法。
...
即 `c=` + `encodeURIComponent(JSON.stringify(models))`，与 owl.js `encodeDataBeforeSend` 完全一致。
...
**对应 owl.js**: `MetricManager.setMetric()` + `report()` 的 POST 上报格式。
...
1. **时间戳单位是秒**而非毫秒：`Math.floor(Date.now() / 1000)`，对齐 owl.js 的 `parseInt(+new Date() / 1000)`
2. **`extra` 是 payload 级别的**：对齐 owl.js `setExtraData()` → `report()` 注入到每条 metric
3. **输入输出字段映射**：
...
**对应 owl.js**: `ResourceModel` 类 + `ResourceManager.send()` 的三种编码方式。

### 数据模型

`ResourceModel` 包含 17 个字段，顺序与 owl.js 完全一致：
...
实现了与 owl.js 完全兼容的最小化 protobuf 编码器 (`ProtobufWriter`)，无需外部依赖。
...
**字段 tag 映射**: 完全对齐 owl.js 的 `lenArr`，例如 `project` → tag 10（field 1 wire type 2）、`pageUrl` → tag 18（field 2 wire type 2），依此类推。
...
**对应 owl.js**: `PageManager.parsePageTime()` 的 `perfMap` 点位体系和 `report()` 的 `speed`/`customspeed` 查询参数。

### 点位索引体系

owl.js 使用固定位置数组上报性能数据，`points[索引] = 值`，最终以 `|` 拼接。索引定义如下：
...
`encodeCustomSpeed(model)` 输出自定义点位的 pipe 字符串，对齐 owl.js `customspeed` 参数。

### 上报方式

owl.js 将 `speed` 和 `customspeed` 作为 GET 请求的 URL 查询参数发送，项目标识 (`project`)、页面地址 (`pageurl`) 等元数据作为独立参数，不再嵌入编码字符串内部。
...
**对应 owl.js**: `PvManager.report()` 的 URL query 参数结构。
...
owl.js 使用 GET 请求，所有 PV 数据作为 URL 查询参数发送，不携带请求体。
...
**对应 owl.js**: Perf 插件 + FSP2 Bridge 事件上报。
...
owl.js 的 `Url.stringify` 不做 `encodeURIComponent` 且不处理 fragment。Protocol 的实现更健壮，在实际使用中不影响兼容性（服务端框架通常自动解码）。

---

## 与 owl.js 的对齐关系一览

| 协议模块 | owl.js 对应 | 数据模型 | 编码方式 | 对齐度 |
|---------|------------|---------|---------|--------|
| Error | `OwlError` / `_processError` | ✅ 字段顺序一致 | ✅ `c=` + JSON | 100% |
| Metric | `MetricManager` | ✅ key/vs/tvs/ts | ✅ JSON POST | 100% |
| Resource | `ResourceModel` | ✅ 17 字段一致 | ✅ JSON + Protobuf | 100% |
| Page Speed | `PageManager` / `perfMap` | ✅ 27 点位一致 | ✅ `\|` 拼接 | 100% |
| PV | `PvManager` | ✅ 字段一致 | ✅ URL query | 100% |
| Perf | Perf 插件 / FSP2 Bridge | — (新增) | JSON | 新增模块 |

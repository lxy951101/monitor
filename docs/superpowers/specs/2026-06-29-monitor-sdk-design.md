...
本项目要参考实现，用 TypeScript、PNPM workspace、Vite 重新实现一套监控 SDK。目标不是把打包后的 bundle 原样复制进仓库，而是把 参考实现 的能力源码化、模块化、可测试化，并把域名、接口路径、CDN 地址、正则和默认开关等提取为配置。

用户确认采用完整复刻方向，并要求多子项目拆包。插件需要拆成独立 package，最终由聚合 SDK 组装出协议和行为兼容的默认实例。由于项目名是 monitor，对外命名默认使用 `Monitor`、`MonitorClient`、`monitor`，不默认创建 `Owl`、`owl` 这类变量名。

## 目标

1. 提供与 参考实现 尽量兼容的浏览器监控能力。
2. 用 PNPM workspace 拆分核心、协议、传输、插件、聚合包和本地验证应用。
3. 将原有实现中硬编码的生产/测试域名、上报路径、资源正则、忽略列表抽离到配置包。
...
- 上报路径：`/api/log`、`/api/logts`、`/api/speedts`、`/pbbatchts`、`/batchts`、`/rapi/metricjts`、`/api/pvts`、。
- 默认配置：`autoCatch`、`page`、`SPA`、`resource`、`ajax`、`image`、`error`、`metric`、`perf`、`bridge`、`compat`。
...
- 错误上报：生成 兼容错误 JSON，POST body 为 `c=<encoded JSON array>`。
- 页面测速：生成 `speed`、`customspeed` 竖线数组编码。
- 资源和 API 上报：维护 字段顺序，并实现 JSON batch 与 protobuf batch。
- 自定义指标：生成 `{ tvs, datas }`。
- 页面访问上报：生成 兼容查询参数。
- Perf 指标：生成 `category/env/logs` 结构和自定义指标结构。
...
- 输出秒开成功、失败、超时等状态。
- 浏览器环境走 web transport，容器环境走通用 bridge。
...
- 注册 core 插件和所有默认插件。
- 导出 `Monitor` 默认门面实例。
- 导出 `MonitorClient` 构造器。
- 挂载 `window.Monitor`。
- 接管 `window.monitor` 队列。
- 保留 `Monitor.__version__`、`Monitor.ErrorModel`、`Monitor.MetricManager`。
- 提供 Vite library build 输出 ESM、CJS、IIFE。

## 兼容接口

聚合包默认支持以下 Monitor 接口，方法名保持参考 SDK 的调用习惯：
...
1. 用户调用 `Monitor.start(config)`。
2. `packages/sdk` 创建默认实例并注册全量插件。
3. `CfgManager` 合并默认配置、用户配置和维度信息。
4. Error、Resource、Page、PV、Metric、Perf 插件按配置启动采集。
5. 插件将事件转成 protocol 模型。
6. protocol 序列化为 兼容格式。
7. transport 发送到配置中的真实域名或 mock-server。
8. 传输成功时回写远端 sampling；失败时按对应缓存策略处理。
...
- Perf fsp/ird/shr 的核心计算函数。

集成测试使用演示验证应用和 mock-server：
...
5. 实现 perf fsp、ird、shr、cache 插件。
6. 实现聚合 `sdk` 包和 `window.Monitor/window.monitor` 默认入口。
7. 实现 mock-server。
8. 实现 playground。
9. 跑完整验证并整理使用文档。
...
- 完整完整实现 体量大，必须分阶段交付，否则容易出现一个超大文件或难以测试的模块。
- protobuf batch 是兼容关键点，需要用测试锁定字段顺序和编码结果。
- Perf 秒开、响应耗时、滚动帧率与浏览器事件循环强相关，单元测试只覆盖核心计算，真实行为需要 playground 冒烟验证。
- 容器 bridge 在普通浏览器不可真实验证，mock-server 和桥 mock 只能验证调用参数。

## 验收标准

1. `packages/sdk` 能以 IIFE 方式挂载 `window.Monitor` 并执行 `window.monitor` 队列。
2. apps/playground 能触发所有核心采集项。
3. apps/mock-server 能接收并解析所有主要 Perf 上报。
4. 默认域名、路径都来自 `packages/config`。

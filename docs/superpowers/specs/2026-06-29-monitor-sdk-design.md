...
- 默认配置：`autoCatch`、`page`、`SPA`、`resource`、`ajax`、`image`、`error`、`metric`、`logan`、`perf`、`bridge`、`compat`。
...
- 错误上报：生成 OWL 兼容错误 JSON，POST body 为 `c=<encoded JSON array>`。
- 页面测速：生成 `speed`、`customspeed` 竖线数组编码。
- 资源和 API 上报：维护 OWL 字段顺序，并实现 JSON batch 与 protobuf batch。
- 自定义指标：生成 `{ tvs, datas }`。
- 页面访问上报：生成 OWL 兼容查询参数。
...
- 仅在显式开启 `compat.legacyOwlAlias` 时读取或创建 `window.owl`、`window.Owl`、`window._Owl_` 迁移别名。
...
- 仅在显式开启 `compat.legacyOwlAlias` 时额外分发 `owlErrDetected` 迁移事件。
...
- 仅在显式开启 `compat.legacyOwlAlias` 时挂载 `window.Owl` 和接管 `window.owl`。
